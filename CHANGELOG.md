...
- **Setup reads your `.env` in ordinary dotenv style — and still asks the right question of it.**
  The wizard's "on file" badges and the `complete` gate that lets you out of Setup read `.env`
  through the settings *line editor*, which reports the FIRST assignment where dotenv keeps the last
  and does not recognise an `export ` prefix at all. So a key set with `export`, or an old line left
  above a new one, could show a keyed provider whose very first paid call dies for want of a key — or
  trap a perfectly good install in Setup forever. The same reading decided which LLM provider's key
  to look for, so an `export LLM_PROVIDER=openai` had the wizard checking Anthropic. And the Segmind
  model probe validated `SEGMIND_SEEDANCE25_SLUG="my-slug"` with its quotes still attached — a 404
  that blamed your key for a setup that was fine. All of these now read the file with dotenv's own
  grammar. What Setup asks of it stays a question about your *file*, though: a key exported in the
  shell that launched the studio is not a key this installation has stored, and must not mark a fresh
  install complete and skip the wizard whose whole job is to write that file. A first run with no
  `.env` yet is still seeded from `.env.example`, unchanged.
