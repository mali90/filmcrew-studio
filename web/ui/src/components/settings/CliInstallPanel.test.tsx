...
const cliStatus = (installed: boolean) => ({ provider: 'claude', bin: 'claude', npmPackage: '@anthropic-ai/claude-code', installed, version: installed ? 'claude 9.9.9' : null });
  ...
  it('installs, re-detects, then prompts to sign in', async () => {
    ...
    server.use(
      ...
      http.post('/api/setup/install-cli', () => {
        installed = true;
        return ndjson([
          { type: 'start', provider: 'claude', pkg: '@anthropic-ai/claude-code', command: 'npm install -g @anthropic-ai/claude-code' },
          { type: 'log', stream: 'stdout', line: 'added 1 package' },
  ...
  it('surfaces a failed install with a Retry action', async () => {
    server.use(
      ...
      http.post('/api/setup/install-cli', () => ndjson([
        { type: 'start', provider: 'claude', pkg: '@anthropic-ai/claude-code', command: 'npm ...' },
        { type: 'error', ok: false, code: 1, message: 'npm install exited with code 1', hint: 'permission denied — run it in a terminal' },
      ])),
    );
    renderPanel();
    await userEvent.click(await screen.findByRole('button', { name: /install claude cli/i }));
