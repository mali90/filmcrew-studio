...
Example: "≤9 reference images/job" and no smaller combined-references number → budget 9; two starred characters → floor(9 ÷ 2) = 4 images EACH. Nothing is held back for the opening/seam frame: the ninth slot carries an un-starred pin this idea calls for, and the seam frame rides it only while it is still free. A character with fewer images than their share contributes all of them — never pad with unrelated images. The "smallest set" rule below governs UN-starred picks only, never the starred cast.
